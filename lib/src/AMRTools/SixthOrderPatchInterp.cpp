...
#include "SixthOrderPatchInterp.H"
#include "BoxIterator.H"
#include "NamespaceHeader.H"

//////////////////////////////////////////////////////////////////////////////
// Constructor - set up some defaults
SixthOrderPatchInterp::SixthOrderPatchInterp()
{
  m_defined = false;
...
void SixthOrderPatchInterp::define(
  ...
  for (BoxIterator bit(stencilBox); bit.ok(); ++bit)
    {
      IntVect offset = bit();
      m_stencils(offset, 0) =
        new SixthOrderInterpStencil(offset, m_refineCoarse, m_degree, m_fixedDims);
    }
...
//////////////////////////////////////////////////////////////////////////////
void SixthOrderPatchInterp::setCoarseBox(const Box& a_coarseBox)
{
...
//////////////////////////////////////////////////////////////////////////////
void SixthOrderPatchInterp::setStencil(BaseFab<IntVect>&  a_stencil)
{
  CH_assert(m_defined);
                  ...
                  if (offLo < 0 && offHi >0) // condition means ivc in coarseDomain
                    {
                      if ((offLo >= -m_maxStencilDist) &&
                        ...
                        { // both of these:  very narrow domain, you are in trouble
                          MayDay::Error("SixthOrderFineInterp::define bad boxes");
                        }
                      if (offLo >= -m_maxStencilDist) // -1, -2, or -3
                        dist[idir] = offLo;
                      if (offHi <= m_maxStencilDist) // 1, 2, or 3
                        dist[idir] = offHi;
...
//////////////////////////////////////////////////////////////////////////////
void SixthOrderPatchInterp::interpToFine(/// interpolated solution on this level
                                          FArrayBox&                a_fine,
                                          /// coarse solution
  ...
  for (BoxIterator bit(m_coarseBox); bit.ok(); ++bit)
    {
      ...
      const SixthOrderInterpStencil& stencil =
