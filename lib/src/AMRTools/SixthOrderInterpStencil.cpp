...
#include "SixthOrderInterpStencil.H"
...
SixthOrderInterpStencil::SixthOrderInterpStencil()
{
  m_defined = false;
}
...
//////////////////////////////////////////////////////////////////////////////
// Define the object
void SixthOrderInterpStencil::define(
                                      const IntVect&         a_bdryOffset,
                                      const int&             a_refineCoarse,
                                      const int&             a_degree,
                                      Interval               a_fixedDims)
{ CH_TIME("SixthOrderInterpStencil::define");
  ...
  { CH_TIME("finding stencil cells");
    ...
    // For 6th order, we need a 3x3x3 inner box centered appropriately
    Box innerBox = Box(center - 1*interpUnit,
                       center + 1*interpUnit);
    ...
    // OUTER SET:
    // For 6th order, we need additional cells at distance 2 and 3
    for (int idir = 0; idir < SpaceDim; idir++)
      {
        if ( !m_fixedDims.contains(idir) )
          { // Add cells beyond innerBox, if possible, in dimension idir
            ...
            // Add cells at distance 2
            if (bdryOffsetDir <= 0)
              { // cell available on right at distance 2
                IntVect iv = IntVect::Zero;
                iv[idir] = center[idir] + 2;
                m_coarseBaseIndices.push_back(iv);
              }
            if (bdryOffsetDir >= 0)
              { // cell available on left at distance 2
                IntVect iv = IntVect::Zero;
                iv[idir] = center[idir] - 2;
                m_coarseBaseIndices.push_back(iv);
              }
            // Add cells at distance 3
            if (bdryOffsetDir <= -1)
              { // cell available on right at distance 3
                IntVect iv = IntVect::Zero;
                iv[idir] = center[idir] + 3;
                m_coarseBaseIndices.push_back(iv);
              }
            if (bdryOffsetDir >= 1)
              { // cell available on left at distance 3
                IntVect iv = IntVect::Zero;
                iv[idir] = center[idir] - 3;
                m_coarseBaseIndices.push_back(iv);
              }
...
//////////////////////////////////////////////////////////////////////////////
void SixthOrderInterpStencil::fillFine(
                                        FArrayBox&         a_fineFab,
...
//////////////////////////////////////////////////////////////////////////////
Real SixthOrderInterpStencil::power1d0avg(Real a_lower,
                                           Real a_upper,
                                           int a_ipwr) const
{
...
//////////////////////////////////////////////////////////////////////////////
Real SixthOrderInterpStencil::power1dcoarseind0avg(int a_offset,
                                                    int a_ipwr) const
{
...
//////////////////////////////////////////////////////////////////////////////
Real SixthOrderInterpStencil::power1dfineind0avg(int a_offset,
                                                  int a_ipwr) const
{
